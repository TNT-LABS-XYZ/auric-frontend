...
import { parseBalance, fmt6, formatRuleSummary, formatRuleDetail, isZeroBalance, type Strategy } from './helpers'
...
describe('formatRuleSummary', () => {
  it('formats DCA strategy', () => {
    const s: Strategy = { _id: '1', type: 'dca', status: 'active', amount: '10000000', frequency: 'weekly' }
    expect(formatRuleSummary(s)).toBe('DCA — $10 weekly')
  })

  it('formats price_trigger using buy_below_usd (not buy_below)', () => {
    const s: Strategy = { _id: '2', type: 'price_trigger', status: 'active', buy_below_usd: 2900, amount: '5000000' }
  ...
  it('formats goal using target_xaut (not target)', () => {
    const s: Strategy = { _id: '3', type: 'goal', status: 'active', target_xaut: 1.5, accumulated: '500000', amount: '10000000', frequency: 'daily' }
    const result = formatRuleSummary(s)
  ...
  it('does not produce NaN when target_xaut is missing', () => {
    const s: Strategy = { _id: '3', type: 'goal', status: 'active', accumulated: '0' }
    expect(formatRuleSummary(s)).not.toContain('NaN')
  })
})
...
describe('formatRuleDetail', () => {
  ...
  it('formats price_trigger detail', () => {
    const s: Strategy = { _id: '2', type: 'price_trigger', status: 'active', amount: '5000000' }
    expect(formatRuleDetail(s)).toBe('Buy $5 when triggered')
  })

  it('formats goal detail', () => {
    const s: Strategy = { _id: '3', type: 'goal', status: 'active', amount: '10000000', frequency: 'daily' }

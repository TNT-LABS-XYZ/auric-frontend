...
function strategyProps(s: Strategy): {
  Icon: Icon;
  label: string;
  primary: string;
  meta: string;
  ...
  if (s.type === "dca") {
    const next = s.last_executed_at
      ? new Date(
          new Date(s.last_executed_at).getTime() +
            (s.frequency === "hourly"
              ? 3600000
              : s.frequency === "daily"
                ? 86400000
    ...
    return {
      Icon: ChartLine,
      label: "DCA",
      primary: `$${f6(s.amount).toFixed(2)} / ${s.frequency ?? ""}`,
      meta: `Next: ${next}`,
    };
  }
  ...
  if (s.type === "price_trigger") {
    const lastHit = s.last_executed_at
      ? new Date(s.last_executed_at).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        })
      : "Never";
    return {
      Icon: Lightning,
      label: "Price trigger",
      primary: `< $${s.buy_below_usd?.toLocaleString() ?? "—"}`,
      meta: `Last hit ${lastHit}`,
    };
  }
  ...
  if (s.type === "goal") {
    ...
    return {
      ...
      label: "Goal",
      primary: `${target} XAU₮`,
      meta: `${acc.toFixed(3)} accumulated`,
      progress: pct,
    };
  }
  ...
  return {
    Icon: Robot,
    label: "AI Agent",
    primary: s.label ?? "AI Agent",
    meta:
      pending > 0
        ? `${pending} step${pending === 1 ? "" : "s"} pending`
        : "Monitoring",
  };
}
...
function StrategyRow({
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() =>
          paused ? onResume(strategy._id) : onPause(strategy._id)
        }
        style={{
          display: "flex",
          alignItems: "center",
      ...
      >
        {paused ? (
          <>
            <Play size={10} weight="fill" style={{ marginRight: 3 }} />
            Resume
          </>
        ) : (
          <>
            <Pause size={10} weight="fill" style={{ marginRight: 3 }} />
            Pause
          </>
        )}
      </button>

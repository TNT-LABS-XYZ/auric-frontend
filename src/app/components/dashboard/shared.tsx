...
export function AuricLogomark({ size = 24, color = 'var(--text)' }: { size?: number; color?: string }) {
  const scale = size / 29
  return (
    <svg width={size} height={Math.round(19 * scale)} viewBox="0 0 29 19"
  ...
  return (
    <button
      ...
      style={{
        ...
        color: done ? 'var(--accent)' : hov ? 'var(--text-mid)' : 'var(--text-muted)',
        letterSpacing: '-0.006em',
        transition: 'color 0.12s ease',
      }}
    >
      {done ? <Check size={11} weight="fill" /> : <Copy size={11} weight="fill" />}
      {done ? 'Copied' : 'Copy'}
    </button>
  )
}

...
export function parseBalance(raw: { xaut: unknown; usdt: unknown }): {
  xaut: number;
  usdt: number;
} {
...
export function formatRuleSummary(s: Strategy): string {
  ...
  if (s.type === "price_trigger")
    return `Price trigger — below $${(s.buy_below_usd ?? 0).toLocaleString()}`;
  if (s.type === "goal")
    return `Goal — ${Number(fmt6(s.accumulated)).toFixed(3)} / ${(s.target_xaut ?? 0).toFixed(4)} XAU₮`;
...
export function formatRuleDetail(s: Strategy): string {
  ...
  if (s.type === "price_trigger")
    return `Buy $${fmt6(s.amount)} when triggered`;

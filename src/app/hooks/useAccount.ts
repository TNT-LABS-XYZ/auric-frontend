...
const apiFetchBalances = async (token: string): Promise<{ balance: WalletBalance | null; price: number | null }> => {
  const headers = authHeaders(token)
  const [balRes, priceRes] = await Promise.allSettled([
    axios.get<WalletBalance>(`${API_URL}/wallet/balance`, { headers }).then((r) => r.data),
    axios.get<{ xaut_usd: number }>(`${API_URL}/wallet/price`, { headers }).then((r) => r.data),
  ])
  const raw = balRes.status === 'fulfilled' ? balRes.value : null
  return {
    balance: raw ? { ...raw, xaut: Number(raw.xaut), usdt: Number(raw.usdt) } : null,
...
const apiFetchExecutions = async (token: string): Promise<Execution[]> => {
  try {
    const { data } = await axios.get<Execution[]>(`${API_URL}/executions`, { headers: authHeaders(token) })
    return data
  } catch {
    return []
  }
...
const apiRequestTelegramToken = async (token: string): Promise<string | null> => {
  try {
    const { data } = await axios.post<{ token: string }>(`${API_URL}/telegram/link`, null, { headers: authHeaders(token) })
...
const apiPauseStrategy = async (token: string, id: string): Promise<void> => {
  await axios.post(`${API_URL}/strategies/${id}/pause`, null, { headers: authHeaders(token) })
}

const apiResumeStrategy = async (token: string, id: string): Promise<void> => {
  await axios.post(`${API_URL}/strategies/${id}/resume`, null, { headers: authHeaders(token) })
}

const apiUnlinkTelegram = async (token: string): Promise<void> => {
  await axios.delete(`${API_URL}/telegram/link`, { headers: authHeaders(token) })
}
...
export const useAccount = () => {
  ...
  const loadDashboard = async (token: string) => {
    const [{ balance: b, price: p }, strats, execs, profile] = await Promise.all([
  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      const telegramToken = alreadyLinked ? null : await apiRequestTelegramToken(token)
  ...
  const pauseStrategy = async (id: string) => {
    // Optimistic update
    setStrategies((prev) => prev.map((s) => s._id === id ? { ...s, status: 'paused' } : s))
    try {
      const token = await getAccessTokenSilently()
      await apiPauseStrategy(token, id)
    } catch {
      // Revert on failure
      setStrategies((prev) => prev.map((s) => s._id === id ? { ...s, status: 'active' } : s))
    }
  }
  ...
  const unlinkTelegram = async () => {
    ...
    try {
      ...
      const telegramToken = await apiRequestTelegramToken(token)
      setAccount((prev) => prev ? { ...prev, telegram_token: telegramToken } : null)
    } catch {
      setTelegramLinked(true)
    }
  }
